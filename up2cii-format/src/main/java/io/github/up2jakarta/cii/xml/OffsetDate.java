 ...
 * <p>
 * For example, the value "2nd October 2007 +02:00" can be stored in an {@code OffsetDate}.
 * <p>
 * {@code OffsetDate} and {@link java.time.Instant} all store an instant on the time-line to nanosecond precision.
 * {@code Instant} is the simplest, simply representing the instant.
 * {@code OffsetDate} adds to the instant the offset from UTC/Greenwich, which allows the local date to be obtained.
 * <p>
 * It is intended that {@code Instant} is used to model data
 * in simpler applications. This class may be used when modeling date concepts in
...
public final class OffsetDate implements Temporal, TemporalAdjuster, Comparable<OffsetDate>, Serializable {
     ...
     * This is the local date of midnight at the start of the minimum date
     ...
     * This is the local date just before midnight at the end of the maximum date
     ...
     */
    public static final OffsetDate MAX = OffsetDate.of(LocalDate.MIN, ZoneOffset.MAX);
    ...
    /**
     * Gets a comparator that compares two {@code OffsetDate} instances based solely on the instant.
     * <p>
     ...
     * @see #isEqual
     */
    public static Comparator<OffsetDate> timeLineOrder() {
        return OffsetDate::compareInstant;
    }
     ...
     */
    private static int compareInstant(OffsetDate date1, OffsetDate date2) {
        if (date1.getOffset().equals(date2.getOffset())) {
            return date1.toLocalDate().compareTo(date2.toLocalDate());
        }
        return Long.compare(date1.toEpochSecond(), date2.toEpochSecond());
    }
    ...
    /**
     * Obtains an instance of {@code OffsetDate} from a date and offset.
     * <p>
     * This creates an offset date with the specified local date and offset.
     *
     * @param date   the local date, not null
     * @param offset the zone offset, not null
     ...
     * <p>
     * {@code LocalDate} has five additional convenience variants of the
    ...
    public static OffsetDate of(int year, int month, int dayOfMonth, ZoneOffset offset) {
        ...
        return new OffsetDate(dt, offset);
    }
    ...
    public static OffsetDate ofInstant(Instant instant, ZoneId zone) {
        ...
        var epochSec = instant.getEpochSecond() + offset.getTotalSeconds();
        var epochDay = Math.floorDiv(epochSec, SECONDS_PER_DAY);
     ...
     * which this factory converts to an instance of {@code OffsetDate}.
     * <p>
     ...
     * {@link java.time.format.DateTimeFormatter#ISO_OFFSET_DATE}.
     *
     ...
     * values based on this date, except {@code NANO_OF_DAY}, {@code MICRO_OF_DAY},
    ...
    public long getLong(TemporalField field) {
        if (field instanceof ChronoField cf) {
            if (cf == INSTANT_SECONDS) {
                return toEpochSecond();
            } else if (cf == OFFSET_SECONDS) {
                return getOffset().getTotalSeconds();
            } else {
     ...
     * <p>
     * This method returns an object with the specified {@code ZoneOffset} and a {@code LocalDate}
     ...
     */
    public OffsetDate withOffsetSameInstant(ZoneOffset offset) {
        if (offset.equals(this.offset)) {
            return this;
        }
        return new OffsetDate(this.date, offset);
    }
     ...
     * Key date classes also implement the {@code TemporalAdjuster} interface,
     ...
     * <p>
     * The {@code INSTANT_SECONDS} field will return a date with the specified instant.
    ...
    public OffsetDate with(TemporalField field, long newValue) {
        if (field instanceof ChronoField cf) {
            if (cf == INSTANT_SECONDS) {
                return ofInstant(Instant.ofEpochSecond(newValue, 0), offset);
            } else if (cf == OFFSET_SECONDS) {
                return with(date, ZoneOffset.ofTotalSeconds(cf.checkValidIntValue(newValue)));
            } else {
     ...
     * <p>
     * If the resulting {@code OffsetDate} is invalid, an exception is thrown.
     * The time and offset do not affect the calculation and will be the same in the result.
     * <p>
     * This instance is immutable and unaffected by this method call.
     *
     ...
     * <p>
     * This returns an {@code OffsetDate}, based on this one, with the specified amount subtracted.
     * The amount is typically {@link Period} or {@link Duration} but may be
     * any other type implementing the {@link TemporalAmount} interface.
     * <p>
     * The calculation is delegated to the amount object by calling
     ...
     * two times, passing {@link ChronoField#EPOCH_DAY} and {@link ChronoField#OFFSET_SECONDS} as the fields.
     * <p>
     ...
     * <p>
     * This calculates the amount of time between two {@code OffsetDate}
     ...
     * If the offset differs between the two dates, the specified
     * end date is normalized to have the same offset as this date.
     * <p>
     ...
     * The units {@code DAYS}, {@code WEEKS}, {@code MONTHS}, {@code YEARS}, {@code DECADES},
     ...
     * This instance is immutable and unaffected by this method call.
     *
     * @param endExclusive the end date, exclusive, which is converted to an {@code OffsetDate}, not null
    ...
    /**
     * Converts this date to the number of seconds from the epoch of 1970-01-01T00:00:00Z.
     * <p>
     * This allows this date to be converted to a value of the
    ...
    public long toEpochSecond() {
        var secs = date.toEpochDay() * SECONDS_PER_DAY;
     ...
     * <p>
     * The comparison is based on the instant then on the local date.
    ...
    public int compareTo(OffsetDate other) {
        ...
        if (cmp == 0) {
            cmp = toLocalDate().compareTo(other.toLocalDate());
        }
        return cmp;
    }

    /**
     * Checks if the instant of this date is after that of the specified date.
     * <p>
     * This method differs from the comparison in {@link #compareTo} and {@link #equals} in that it
     * only compares the instant of the date.
     *
     * @param other the other date to compare to, not null
    ...
    /**
     * Checks if the instant of this date is before that of the specified date.
     * <p>
     * This method differs from the comparison in {@link #compareTo} in that it only compares the instant of the date.
     *
     * @param other the other date to compare to, not null
    ...
    /**
     * Checks if the instant of this date is equal to that of the specified date.
     * <p>
    ...
    public boolean equals(Object obj) {
        ...
        if (obj instanceof OffsetDate other) {
