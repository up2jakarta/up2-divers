...
public class DocumentStatusCodeTest extends CodeAdapterTest {

    private static final io.github.up2jakarta.cii.format.unmapped.qdt.DocumentStatusCodeType WRONG_CODE =
            new io.github.up2jakarta.cii.format.unmapped.qdt.DocumentStatusCodeType() {{
                setValue("???");
            }};

    public DocumentStatusCodeTest(@Autowired ApplicationContext context) throws IOException {
        super(context, "invalid_document_status.xml", (i) -> {
            var trade = i.getSupplyChainTradeTransaction();
            var agreement = trade.getApplicableHeaderTradeAgreement();
            var order = agreement.getSellerOrderReferencedDocument();
            order.setStatusCode(WRONG_CODE);
        });
    }
    ...
    public void value() {
        ...
        final ReferencedDocumentType order = agreement.getSellerOrderReferencedDocument();
        assertNotNull(order);
        final DocumentStatusCodeType code = order.getStatusCode();
        assertEquals(DocumentStatusCodeType.V_1, code);
