...
class Tree80UnitTests {

    private final SimpleUnitImporter<Tree80, TermType, SegmentType> importer;

    @Autowired
    Tree80UnitTests(Up2Factory<TermType> factory) throws BeanException {
        this.importer = new SimpleUnitImporter<>(factory, Tree80.class, SegmentType.class);
    }
    ...
    void testLevel4() {
        ...
        final UnitRecord<SegmentType>[] records = new UnitRecord[]{
                ...
                record("82", "10", "20", "$.10.20"),
                record("83", "10", "20", "30", "$.10.20.30"),
                record("84", "10", "20", "30", "40", "$.10.20.30.40")
        };
        final List<PropertyEvent<?, ?>> errors = new LinkedList<>();
        // When & Then
        final Tree80 root = importer.parse(records, (i, r) -> {
    ...
    void testDetached2() {
        // Given
        final UnitRecord<SegmentType> detached = record("82", "11", "21", "$.10.20");
        final UnitRecord<SegmentType>[] records = new UnitRecord[]{
                record("80", "00", "$"),
                record("81", "10", "$.10"),
                record("82", "10", "20", "$.10.20"),
    ...
    @Test
    void testDetached3() {
        final UnitRecord<SegmentType> detached1 = record("83", "11", "20", "31", "$.11.20.31");
        final UnitRecord<SegmentType> detached2 = record("83", "10", "21", "31", "$.10.21.31");
    ...
    void testDetached4() {
        // Given
        final UnitRecord<SegmentType> detached1 = record("84", "11", "20", "30", "41", "$.11.20.30.41");
        final UnitRecord<SegmentType> detached2 = record("84", "10", "21", "30", "41", "$.10.21.30.41");
        final UnitRecord<SegmentType> detached3 = record("84", "10", "20", "31", "41", "$.10.20.31.41");
        final List<Object> failures = new ArrayList<>(2);
        {
            failures.add(detached1);
            failures.add(detached2);
    ...
    void testExportReferenceIds() throws BeanException, IOException {
        ...
        importer.toExporter().format(root, row -> {
            ...
            assertEquals(src.length, row.length - 1);
            for (int i = 1; i < row.length; i++) {
                assertEquals(row[i], src[i - 1]);
            }
            store.remove(found);
        });
