...
class Tree80FastTests {

    private final SimpleFastImporter<Tree80, TermType, SegmentType> importer;

    @Autowired
    Tree80FastTests(Up2Factory<TermType> factory) throws BeanException {
        this.importer = new SimpleFastImporter<>(factory, Tree80.class, SegmentType.class);
    }

    private FastRecord<SegmentType> record(SegmentType type, String... data) throws CodeListException {
        return new FastRecord<>(type, null, data);
    }
    ...
    void testDetached2() {
        // Given
        final FastRecord<SegmentType> detached = record(S82, "11", "21", "$.10.20");
        final FastRecord<SegmentType>[] records = new FastRecord[]{
                record(S80, "00", "$"),
                record(S81, "10", "$.10"),
                record(S82, "10", "20", "$.10.20"),
    ...
    @Test
    void testDetached3() {
        final FastRecord<SegmentType> detached1 = record(S83, "11", "20", "31", "$.11.20.31");
        final FastRecord<SegmentType> detached2 = record(S83, "10", "21", "31", "$.10.21.31");
    ...
    void testDetached4() {
        // Given
        final FastRecord<SegmentType> detached1 = record(S84, "11", "20", "30", "41", "$.11.20.30.41");
        final FastRecord<SegmentType> detached2 = record(S84, "10", "21", "30", "41", "$.10.21.30.41");
        final FastRecord<SegmentType> detached3 = record(S84, "10", "20", "31", "41", "$.10.20.31.41");
        final List<Object> failures = new ArrayList<>(2);
        {
            failures.add(detached1);
            failures.add(detached2);
