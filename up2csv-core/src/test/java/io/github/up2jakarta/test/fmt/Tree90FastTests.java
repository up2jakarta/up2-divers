...
import io.github.up2jakarta.csv.fmt.SimpleFastExporter;
import io.github.up2jakarta.csv.fmt.SimpleFastImporter;
import io.github.up2jakarta.lov.CodeListException;
import io.github.up2jakarta.lov.core.BeanException;
import io.github.up2jakarta.test.TUConfiguration;
import io.github.up2jakarta.test.fmt.misc.AFastTester;
...
class Tree90FastTests {

    private final SimpleFastImporter<Tree90, TermType, SegmentType> importer;
    private final SimpleFastExporter<Tree90, TermType, SegmentType> exporter;

    @Autowired
    Tree90FastTests(Up2Factory<TermType> factory) throws BeanException {
        this.importer = new SimpleFastImporter<>(factory, Tree90.class, SegmentType.class);
    ...
    void testLevel0() throws IOException {
        // Given
        final FastRecord<SegmentType>[] records = new FastRecord[]{
                record("90", "00", "$")
        };
        final List<PropertyEvent<?, ?>> errors = new LinkedList<>();
        // When & Then
        ...
        // When Formating
        final AFastTester<FastRecord<SegmentType>> tc = new AFastTester<>(root, records);
    ...
    void testLevel3() throws IOException {
        ...
        final FastRecord<SegmentType>[] records = new FastRecord[]{
                ...
                record("91", "00", "10", "$.10"),
                record("92", "00", "10", "20", "$.10.20"),
                record("93", "00", "10", "20", "30", "$.10.20.30")
        };
        final List<PropertyEvent<?, ?>> errors = new LinkedList<>();
        // When & Then
        final Tree90 root = importer.parse(records, (i, r) -> {
    ...
    void testLevel4() throws IOException {
        ...
        final FastRecord<SegmentType>[] records = new FastRecord[]{
                ...
                record("92", "00", "10", "20", "$.10.20"),
                record("93", "00", "10", "20", "30", "$.10.20.30"),
                record("94", "00", "10", "20", "30", "40", "$.10.20.30.40")
        };
        final List<PropertyEvent<?, ?>> errors = new LinkedList<>();
        // When & Then
        final Tree90 root = importer.parse(records, (i, r) -> {
    ...
    void testFastDetached() {
        // Given
        final FastRecord<SegmentType> detached = record("92", "00");
        final FastRecord<SegmentType>[] records = new FastRecord[]{
                record("90", "00", "$"),
                record("91", "00", "10", "$.10"),
                detached
    ...
    void testDetached2() {
        // Given
        final FastRecord<SegmentType> detached = record("92", "00", "11", "21", "$.10.20");
        final FastRecord<SegmentType>[] records = new FastRecord[]{
                record("90", "00", "$"),
                record("91", "00", "10", "$.10"),
                record("92", "00", "10", "20", "$.10.20"),
    ...
    @Test
    void testDetached3() {
        final FastRecord<SegmentType> detached1 = record("93", "00", "11", "20", "31", "$.11.20.31");
        final FastRecord<SegmentType> detached2 = record("93", "00", "10", "21", "31", "$.10.21.31");
    ...
    void testDetached4() {
        // Given
        final FastRecord<SegmentType> detached1 = record("94", "00", "11", "20", "30", "41", "$.11.20.30.41");
        final FastRecord<SegmentType> detached2 = record("94", "00", "10", "21", "30", "41", "$.10.21.30.41");
        final FastRecord<SegmentType> detached3 = record("94", "00", "10", "20", "31", "41", "$.10.20.31.41");
        final List<Object> failures = new ArrayList<>(2);
        {
            failures.add(detached1);
            failures.add(detached2);
    ...
    void testImportValidation() throws IOException {
        ...
        final FastRecord<SegmentType>[] records = new FastRecord[]{
                record("90", "00", null),
                record("91", "00", "10", null),
                record("92", "00", "10", "20", null),
                record("93", "00", "10", "20", "30", null),
                record("94", "00", "10", "20", "30", "40", null)
        };
        final List<PropertyEvent<?, ?>> errors = new LinkedList<>();
        // When Parsing
        final Tree90 root = importer.parse(records, (i, r) -> {
        ...
        for (var level = 0; level < errors.size(); level++) {
            ...
            assertEquals(level + 2, error.getOffset());
        }
        // When Formating
        final AFastTester<FastRecord<SegmentType>> tc = new AFastTester<>(root, records);
        exporter.format(root, tc::assertExists);
    ...
    void testExportValidation() {
        ...
        final FastRecord<SegmentType>[] records = new FastRecord[]{
                ...
                record("91", "00", "10"),
                record("92", "00", "10", "20"),
                record("93", "00", "10", "20", "30"),
                record("94", "00", "10", "20", "30", "40")
        };
    ...
    void testPIdValidation() {
        // Given
        final FastRecord<SegmentType> detached = record("92", "00", "\t", "20", "$.10.20");
        final FastRecord<SegmentType>[] records = new FastRecord[]{
                record("90", "00", "$"),
                record("91", "00", "10", "$.10"),
                detached
