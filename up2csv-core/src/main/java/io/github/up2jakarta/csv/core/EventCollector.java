...
import static io.github.up2jakarta.csv.core.Errors.getErrorCode;
import static io.github.up2jakarta.csv.core.Errors.getSeverity;

/**
 * Base implementation of {@link EventHandler} that collects events during the mapping, validation and parsing phases.
 */
public abstract class EventCollector<R extends IRecord<?>, D extends DataType<D>, E extends IError<D>> extends EventHandler<R, D, E> {

    private final Set<Integer> offsets = new LinkedHashSet<>();

    protected EventCollector(R row) {
        super(row);
    }
    ...
    public final void handleEvent(D data, int offset, Exception cause, Error config) {
        final String code = getErrorCode(cause, config);
        final SeverityType type = getSeverity(cause, config);
