 ...
 * on binary-search tree with weak-reference values.
 *
 * @param <K> the key type
 * @param <V> the value type
 */
public non-sealed class WeakValueMap<K, V> extends KVTree<K, V> {

    private final ReferenceQueue<V> queue = new ReferenceQueue<>();

    public WeakValueMap(Comparator<? super K> comparator) {
        super(comparator);
    }
    ...
    public final Entry<K, V> update(Entry<K, V> entry, V value) {
        ...
        if (entry instanceof WVEntry<K, V> node) {
            this.detach(node);
        } else {
            this.delete(key);
        }
        return this.insert(key, () -> new WVEntry<>(key, value, queue));
    }
    ...
    final Immutable<K, V> create(K key, V value) {
        return new WVEntry<>(key, value, queue);
    }

    static final class WVEntry<K, V> extends WeakReference<V> implements Immutable<K, V> {
        private final K key;
        private boolean color = true;
        private Node<K, KVNode<K, V>> parent, right, left;

        private WVEntry(K key, V value, ReferenceQueue<V> queue) {
            super(value, queue);
            this.key = key;
